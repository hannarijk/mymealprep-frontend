...
import { RotateCcw, Trash2, Eye } from 'lucide-vue-next'
...
function handleViewPlan(plan: MealPlan) {
  selectedPlan.value = plan
}

async function handleReuseFromDrawer(plan: MealPlan) {
        ...
        >
          <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0">
              ...
              <!-- Source plan -->
              <p v-if="plan.sourcePlanId" class="mt-1.5 text-xs text-slate-400">
                ↩ from
                {{
                  historyStore.history.find(p => p.id === plan.sourcePlanId)?.title ??
                  'another plan'
                }}
              </p>

              <!-- Notes -->
              <p v-if="plan.notes" class="mt-1.5 text-sm italic text-slate-500">
                "{{ plan.notes }}"
              </p>
            </div>
            ...
            <div class="flex shrink-0 items-center gap-2">
              <button
                class="flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50"
              ...
              <span
                v-else
                class="flex items-center gap-1.5 rounded-xl bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-700"
              >
                Active plan
              </span>
              <button
